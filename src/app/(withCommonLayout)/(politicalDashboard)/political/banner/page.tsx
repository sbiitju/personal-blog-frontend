...
const AllBanner = () => {
  ...
  return (
    <div className="w-full px-4 py-6">
      ...
      {isLoading && <p className="text-center text-gray-600">Loading banners...</p>}
          ...
          <Table className="w-full lg:w-[500px]">
            ...
            <TableBody>
              {bannerData?.data?.length > 0 ? (
                bannerData.data.map((banner: { _id: string; image: string }, key: number) => (
                  <TableRow key={banner._id}>
                    <TableCell className="text-center font-medium">{key + 1}</TableCell>
